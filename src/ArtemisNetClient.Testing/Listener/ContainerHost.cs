...
namespace ActiveMQ.Artemis.Client.Testing.Listener
{
    ...
    internal class ContainerHost : IContainerHost
    {
        ...
        Amqp.Message IContainer.CreateMessage(ByteBuffer buffer)
        {
            return Amqp.Message.Decode(buffer);
        }
        ...
        Link IContainer.CreateLink(ListenerConnection connection, ListenerSession session, Attach attach)
        {
            ...
            link.AddClosedCallback(this.onLinkClosed);
            return link;
        }
        ...
        bool IContainer.AttachLink(ListenerConnection connection, ListenerSession session, Link link, Attach attach)
        {
            ...
            string address = null;
            if (attach.Role)
                address = ((Source) attach.Source).Address;
            else if (attach.Target is Target target)
            {
            ...
            if (this.linkProcessor != null)
            {
                var constructorInfo = typeof(AttachContext).GetTypeInfo().DeclaredConstructors.First();
                var attachContext = constructorInfo.Invoke(new object[] {listenerLink, attach}) as AttachContext;
                this.linkProcessor.Process(attachContext);
                return false;
            }
        ...
        abstract class Collection<T> : IDisposable
        {
            ...
            {
                link.AddSafeClosed(this.OnLinkClosed);
            ...
            void IDisposable.Dispose()
            {
                ...
                lock (this.collection)
                {
                    ...
                    {
                        link.Close(TimeSpan.Zero, new Error(ErrorCode.DetachForced)
                        {
        ...
        {
            static readonly Action<ListenerLink, Amqp.Message, DeliveryState, object> dispatchRequest = DispatchRequest;
            ...
            static void DispatchRequest(ListenerLink link, Amqp.Message message, DeliveryState deliveryState, object state)
            {
                ...
                RequestContext context = (RequestContext) Activator.CreateInstance(typeof(RequestContext), link, responseLink, message)!;
            ...
            void IDisposable.Dispose()
            {
                ...
                lock (this.requestLinks)
                {
                    ...
                    {
                        this.requestLinks[i].Close(TimeSpan.Zero, error);
                    }
